    ...
    for(i = 0; i < firmNum; i++) {
		 ...
		 newFirm.setSalary(salary); // 5/12/16
     	 for (j=0; j<numHedonicElements; j++) { //numHedonicElements is declared in firm.h
             ...
             newFirm.setArtificialQualityElementJ(j,0); // 2026-04-15 copilot: artificial hedonic quality starts at zero.
         }
...
// 2026-04-15 copilot: effective utility mirrors standard utility but uses effective hedonic qualities.
//calculate effective utility using effective hedonic quality (= standard + artificial)
double calcEffectiveUtility(firm firms[], double income, const int firmNum,
			double shares[], std::ofstream &testoutput, int round, bool anotation, bool cesHedonics,
            double cesHedonicsElastGoods, double cesHedonicsElastChars, double complementIntConstraintFactor) {

	int ii,jj,kk;
	double *repConsumption;
	repConsumption = new double[firmNum];
	double *repHedonicConsumption;
	repHedonicConsumption = new double[numHedonicElements];
	double repUtility = 0.0;
	double rhoOneInverse = 0.0;
	double rhoTwoInverse = 0.0;

	rhoOneInverse = 1.0/cesHedonicsElastGoods;
	rhoTwoInverse = 1.0/cesHedonicsElastChars;

	for(ii=0;ii<firmNum;ii++) {
		repConsumption[ii] = (income * shares[ii])/firms[ii].getPrice();
	}

	for(jj=0; jj<numHedonicElements; jj++) {
		double repHedCons = 0.0;
		for(ii=0; ii<firmNum; ii++) {
            if (cesHedonics) {
               double zz = 0;
               zz += ((double) firms[ii].getEffectiveQualityElementJ(jj)) * repConsumption[ii];
               if(firms[ii].getHasComplements()) {
                  for(kk=0;kk<firms[ii].getNumComplements();kk++) {
                     double overlap = 0;
                     if(repConsumption[ii] < repConsumption[firms[ii].getComplementaryProductK(kk)])
                         overlap = floor((1/complementIntConstraintFactor)*repConsumption[ii])*complementIntConstraintFactor;
                     else
                         overlap = floor((1/complementIntConstraintFactor)*repConsumption[firms[ii].getComplementaryProductK(kk)])*complementIntConstraintFactor;
                     zz += ((double) firms[ii].getEffectiveComplementaryHedonicElementKJ(kk,jj)) * overlap;
                  }
               }
			   repHedCons += (pow(((double) zz), cesHedonicsElastGoods));
            }
            else {
               repHedCons += (((double) firms[ii].getEffectiveQualityElementJ(jj)) * repConsumption[ii]);
			   ...
			   if(firms[ii].getHasComplements()) {
                  for(kk=0;kk<firms[ii].getNumComplements();kk++) {
                     ...
                     if(repConsumption[ii] < repConsumption[firms[ii].getComplementaryProductK(kk)])
                         ...
                         overlap = floor(repConsumption[firms[ii].getComplementaryProductK(kk)]);
                     repHedCons += (((double) firms[ii].getEffectiveComplementaryHedonicElementKJ(kk,jj)) * overlap);
                  }
               }
            }
		}
		if(cesHedonics) {repHedonicConsumption[jj] = (1/((double) firmNum))*pow(repHedCons, rhoOneInverse);}
		else {repHedonicConsumption[jj] = repHedCons;}

		repUtility += pow(repHedonicConsumption[jj], cesHedonicsElastChars);
	}
	repUtility = (1/((double) numHedonicElements))*pow(repUtility, rhoTwoInverse);

    delete[] repConsumption;
    repConsumption = NULL;
    delete[] repHedonicConsumption;
    repHedonicConsumption = NULL;
...
double doConsumerSearch(firm firms[], double wageBill, double salaryBill, const int firmNum, /*const int numHedonicElelments,*/
    ...
    double effectiveUtility = 1.0;
    double altEffectiveUtility = 1.0;
	...
	if (round >= 1) {		
        ...
        effectiveUtility = calcEffectiveUtility(firms, income, firmNum, shares, testoutput, round, anotation, cesHedonics, cesHedonicsElastGoods, cesHedonicsElastChars, complementIntConstraintFactor);
		/*
        if(round == 9999 ) {
		...
		*/
        ...
        for(ss=0;ss<numSearchTrials;ss++) { //11/15/10 //add here to have multiple tests as before
            ...
            if (negCount == 0) {
               ...
               altEffectiveUtility = calcEffectiveUtility(firms, income, firmNum, altShares, testoutput, round, anotation, cesHedonics, cesHedonicsElastGoods, cesHedonicsElastChars,complementIntConstraintFactor);
               //if(round == 9999) {printf("in round 10000 negCount = 0 \n");}
               //if(round == 9999) {printf("in round 10000 alt utility is %f \n", altUtility );}
            }
			else {altUtility = utility; altEffectiveUtility = effectiveUtility;}
            //if(round == 9999) {printf("in round 10000 alt utility is %f \n", altUtility );}
						
            //switch to altShares (and update utility) if altUtility > utility
            if(altEffectiveUtility >= effectiveUtility) {
		    ...
		    //firms[firm2].setFinalDemandShare(altShares[firm2]); //should do this indirectly via current? no
		       ...
		       utility = altUtility;
               effectiveUtility = altEffectiveUtility;
...
double doConsumerSearchClass1(firm firms[], double wageBill, const int firmNum, /*const int numHedonicElelments,*/
	...
	if (round >= 1) {		
		...
		/*
		if(round==54){
			for(j=0;j<firmNum;j++) {printf("in round 55 init share and altShare of firm %d is %f  %f  \n " , j+1, shares[j], altShares[j]);}	
		}
		*/
...
void updateRandDInvestmentStatusDC(firm firms[], const int firmNum, double intensityOfChoice, double weightRandDDC, double probRandDInvestMutation, bool intermediateGoods, double overHeadLReq, double overHeadRandDReq, double salary, double intPriceDiscount, double &medRecentRandDInvestment, double &avgRecentProfitAboveMedRandD, double &avgRecentProfitBelowMedRandD, double &avgRecentProfitType1, double &avgRecentProfitType2, std::ofstream &testoutput, int round, bool anotation) { //3/21/10
	...
	if(randNumUni() > 1 - weightRandDDC){ //5/28/16 condition on relative overall R&D profitability
        ...
        if(avgRecentProfitType1 > avgRecentProfitType2) { //5/28/16
		     for(i = 0; i<firmNum; i++) {
		     	if(randNumUni() > (2 - 2 * discChoiceProb2)){  //fixed error 2/9/17 (prob2 not prob1)
        ...
        if(avgRecentProfitType1 < avgRecentProfitType2) { //changed 4/6/16
		     for(i = 0; i<firmNum; i++) {
		     	if(randNumUni() > (2 * discChoiceProb2)) {  //fixed error 2/9/17 (prob2 not prob1)
                   	if(firms[i].getType()==1 && firms[i].getRandDInvestment()==true) {
                        firms[i].setRandDInvestment(false);
              			//if(i==4){printf("firm %d has random innovation switch in round %d to %d \n",i,round,firms[i].getRandDInvestment());} //3/24/10 testing	
		        	}
                    if(firms[i].getType()==2 && firms[i].getRandDInvestment()==false) {
                        //if(randNumUni() > (2 * discChoiceProb1)) { //2/3/17 this line commented out as was in error
                        	firms[i].setRandDInvestment(true);
              				//if(i==4){printf("firm %d has random innovation switch in round %d to %d \n",i,round,firms[i].getRandDInvestment());} //3/24/10 testing	
		                //} //2/3/17 
                   	} 
...
//depreciation of artificial quality and conversion from hedonic to artificial quality -- 2026-04-25
void doDepreciationAndConversion(firm firms[], const int firmNum, double depreciation_rate, int conversion_threshold, double conversion_rate, std::ofstream &testoutput, int round, bool anotation) {
    ...
    for(i = 0; i < firmNum; i++) {
        ...
        //DEPRECIATION: for each positive artificial hedonic quality element, decrement by 1 with probability depreciation_rate
        for(j = 0; j < numHedonicElements; j++) {
            int artval = firms[i].getArtificialQualityElementJ(j);
            if(artval > 0 && randNumUni() < depreciation_rate) {
                firms[i].setArtificialQualityElementJ(j, artval - 1);
            }
        }
        
        //DEPRECIATION: for complementary artificial hedonic elements
        for(k = 0; k < maxNumComplements; k++) {
            for(j = 0; j < numHedonicElements; j++) {
                int artval = firms[i].getArtificialComplementaryHedonicElementKJ(k, j);
                if(artval > 0 && randNumUni() < depreciation_rate) {
                    firms[i].setArtificialComplementaryHedonicElementKJ(k, j, artval - 1);
                }
            }
        }
        
        //CONVERSION: for each hedonic quality element above threshold, move 1 unit to artificial quality with probability conversion_rate
        for(j = 0; j < numHedonicElements; j++) {
            int hedval = firms[i].getHedonicQualityElementJ(j);
            if(hedval > conversion_threshold && randNumUni() < conversion_rate) {
                firms[i].setHedonicQualityElementJ(j, hedval - 1);
                int artval = firms[i].getArtificialQualityElementJ(j);
                firms[i].setArtificialQualityElementJ(j, artval + 1);
            }
        }
        
        //CONVERSION: for complementary hedonic elements above threshold
        for(k = 0; k < maxNumComplements; k++) {
            for(j = 0; j < numHedonicElements; j++) {
                int hedval = firms[i].getComplementaryHedonicElementKJ(k, j);
                if(hedval > conversion_threshold && randNumUni() < conversion_rate) {
                    firms[i].setComplementaryHedonicElementKJ(k, j, hedval - 1);
                    int artval = firms[i].getArtificialComplementaryHedonicElementKJ(k, j);
                    firms[i].setArtificialComplementaryHedonicElementKJ(k, j, artval + 1);
                }
            }
...
}       //end doDepreciationAndConversion
         ...
         if(current.getCapital() < 0.0  || randNumUni() < probRandomRestart) { //with random failure 0.0005
                ...
                if(imitation  && comparison.getTechEff() >= 0.5) { //combined 7/29/09
					...
					for (j=0; j<numHedonicElements; j++) {
                         ...
                         newFirm.setArtificialQualityElementJ(j,0); // 2026-04-15 copilot: restart artificial quality at zero.
					}
					if(randNumUni() > 1 - probIntegerConstrained) {newFirm.setIntegerConstrained(true);}
				...
				newFirm.setCapital(100.0); //7-12-11 //2-28-26 change from 100 to 5 temp 
...
int doRestartsHedonicsEndogN1N2(firm firms[], const int firmNum, int &firmNum1, int &firmNum2, double strtTech, 
    ...
    for(i = 0; i < firmNum; i++) {
         ...
         if(current.getCapital() < 0.0  || randNumUni() < probRandomRestart) { //with random failure 0.0005
                ...
                //uncomment this if want to see when firm 73 restarts //4/25/26
				//if(i==72) { //5
				//	printf("firm 73 has restarted \n" );
...
//consumer optimization for class 1 (PL workers) -- maximize hedonic utility over fixed firm qualities //copilot 2026-05-10
double doPostRunConsumerOptimizationClass1(firm firms[], double wageBill, const int firmNum, int optimRounds, std::ofstream &output4opt, 
			int numMutationPairs, int numSearchTrials, double mutScale, bool anotation, bool cesHedonics, 
            double cesHedonicsElastGoods, double cesHedonicsElastChars, double complementIntConstraintFactor) {
	
	double utility = 1.0;
	double altUtility = 1.0;
	double *shares;
	shares = new double[firmNum];
	double *altShares;
	altShares = new double[firmNum];
    
	double income = wageBill;
	
	//Start with current shares for type 1 firms, zero for type 2
	int i,j;
	for(i=0;i<firmNum;i++) {
		if (firms[i].getType()==1) {
			shares[i] = firms[i].getFinalDemandShare();
	...
	//Calculate initial utility using hedonic qualities ONLY
	std::ofstream dummyOutput; //dummy output stream since we don't want optimization output
	utility = calcUtility(firms, income, firmNum, shares, dummyOutput, 0, false, cesHedonics, cesHedonicsElastGoods, cesHedonicsElastChars, complementIntConstraintFactor);
	
	//Run optimization loop for optimRounds iterations
	for(int optRound = 0; optRound < optimRounds; optRound++) {
		int negCount = 0;
		
		// Run mutation pairs loop
		for(int ss=0; ss<numSearchTrials; ss++) {
			int negCount_trial = 0;
			
			for(int s=0; s<numMutationPairs; s++) {
				double mutFactor = 0.0;
				int sss = 0;
				int integerCase = 0;
				int firm3 = 0;
				...
				// Pick two firms to consider changing shares
				int firm1 = rand() % firmNum;
				int firm2 = rand() % firmNum;
				
				// Make sure they are type 1
				while(firms[firm1].getType()!=1) {firm1 = rand() % firmNum;}
				while(firms[firm2].getType()!=1) {firm2 = rand() % firmNum;}
				
				// Make sure they are different and of the right type and try for positive share
				while((firm2 == firm1 || altShares[firm2] == 0 || firms[firm2].getType()!=1) && sss < 50) {
					firm2 = rand() % firmNum;
					sss++;
				}
				...
				// Make sure both firms are type 1
				if(firms[firm1].getType()==1 && firms[firm2].getType()==1) {
					if(integerCase == 1) { //neither firm is integer constrained
						mutFactor = randNumUni()*mutScale;
						if(mutFactor > 1- altShares[firm1]) {mutFactor = 1 - altShares[firm1];}
						if(mutFactor > altShares[firm2]) {mutFactor = altShares[firm2];} 
						altShares[firm1] = altShares[firm1] + mutFactor;
						altShares[firm2] = altShares[firm2] - mutFactor;
					...
					if(integerCase == 2) { //firm1 is integer constrained
						mutFactor = firms[firm1].getPrice() / income;
						if(mutFactor < 1- altShares[firm1]) {
							altShares[firm1] = altShares[firm1] + mutFactor;
							for(int k=0; k<10; k++) {
								firm3 = rand() % firmNum;
								altShares[firm3] = altShares[firm3] - mutFactor/10.0;
								if (altShares[firm3] < 0) {++negCount_trial;}
							}
						}
					}
					
					if(integerCase == 3) { //firm2 is integer constrained
						mutFactor = firms[firm2].getPrice() / income;
						if(mutFactor < altShares[firm2]) {
							altShares[firm2] = altShares[firm2] - mutFactor;
							for(int k=0; k<10; k++) {
								firm3 = rand() % firmNum;
			...
			} // end mutation pairs loop
			
			// Evaluate alternative shares
			if (negCount_trial == 0) {
				altUtility = calcUtility(firms, income, firmNum, altShares, dummyOutput, 0, false, cesHedonics, cesHedonicsElastGoods, cesHedonicsElastChars, complementIntConstraintFactor);
			}
			else {
				altUtility = utility;
			}
			
			// Accept improvement (comparing HEDONIC utility, not effective utility)
			if(altUtility >= utility) {
				for(j=0; j<firmNum; j++) {
					if(firms[j].getType()==1) {
						firms[j].setFinalDemandShare(altShares[j]);
					}
				}
				utility = altUtility;
			...
			// Reset altShares for next trial
			for(j=0; j<firmNum; j++) {
				if(firms[j].getType()==1) {
					altShares[j] = firms[j].getFinalDemandShare();
				}
			}
		} // end search trials loop
	} // end optimization rounds loop
...
//consumer optimization for class 2 (OH workers) -- maximize hedonic utility over fixed firm qualities //copilot 2026-05-10
double doPostRunConsumerOptimizationClass2(firm firms[], double salaryBill, const int firmNum, int optimRounds, std::ofstream &output4opt,
			int numMutationPairs, int numSearchTrials, double mutScale, bool anotation, bool cesHedonics, 
            double cesHedonicsElastGoods, double cesHedonicsElastChars, double complementIntConstraintFactor) {
	...
	double income = salaryBill;
	
	//Start with current shares for type 2 firms, zero for type 1
	int i,j;
	for(i=0; i<firmNum; i++) {
		if(firms[i].getType()==2) {
			shares[i] = firms[i].getFinalDemandShare();
	...
	for(int optRound = 0; optRound < optimRounds; optRound++) {
		...
		for(int ss=0; ss<numSearchTrials; ss++) {
			...
			for(int s=0; s<numMutationPairs; s++) {
				...
				// Make sure they are type 2
				while(firms[firm1].getType()!=2) {firm1 = rand() % firmNum;}
				while(firms[firm2].getType()!=2) {firm2 = rand() % firmNum;}
				
				// Make sure they are different and of the right type and try for positive share
				while((firm2 == firm1 || altShares[firm2] == 0 || firms[firm2].getType()!=2) && sss < 50) {
					firm2 = rand() % firmNum;
					sss++;
				}
				...
				// Make sure both firms are type 2
				if(firms[firm1].getType()==2 && firms[firm2].getType()==2) {
					if(integerCase == 1) { //neither firm is integer constrained
						mutFactor = randNumUni()*mutScale;
						if(mutFactor > 1- altShares[firm1]) {mutFactor = 1 - altShares[firm1];}
						if(mutFactor > altShares[firm2]) {mutFactor = altShares[firm2];} 
