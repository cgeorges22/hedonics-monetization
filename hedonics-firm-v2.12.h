...
public:
  ...
  firm() {
    ...
    capital = 100.0; //100 reset temp to 20 5 2-28-26 // default is 80 // has been 200 // for no restarts set very large
	...
	profit = 0.0;
	recentProfits = 0.0; 
  // 2026-04-15 copilot: initialize standard, artificial, and effective hedonic arrays.
    for(j=0; j<numHedonicElements; j++) {
    hedonicQuality[j] = 0;
    artificialQuality[j] = 0;
    effectiveQuality[j] = 0;
  }
    for(k=0; k<maxNumComplements; k++) {
    complementaryProducts[k] = 0;
  }
    for(k=0; k<maxNumComplements * numHedonicElements; k++) {
    complementaryHedonicElements[k] = 0;
    artificialComplementaryHedonicElements[k] = 0;
    effectiveComplementaryHedonicElements[k] = 0;
  }
 };
  ...
  // 2026-04-15 copilot: expose artificial and effective hedonic quality values.
  //Get the artificial hedonic quality of the firm's good
  int getArtificialQualityElementJ(int j) const {
	assert(j>=0 && j<numHedonicElements);
	return artificialQuality[j];
  }

  //Get the effective hedonic quality (= standard + artificial)
  int getEffectiveQualityElementJ(int j) const {
	assert(j>=0 && j<numHedonicElements);
	return effectiveQuality[j];
  }
  ...
  //Get the artificial complementary hedonic quality
  int getArtificialComplementaryHedonicElementKJ(int k, int j) const {
  assert(k>=0 && k<maxNumComplements);
  assert(j>=0 && j<numHedonicElements);
  assert(k * numHedonicElements + j >=0 && k * numHedonicElements + j < maxNumComplements * numHedonicElements);
  return artificialComplementaryHedonicElements[k * numHedonicElements + j];
  }

  //Get the effective complementary hedonic quality (= standard + artificial)
  int getEffectiveComplementaryHedonicElementKJ(int k, int j) const {
  assert(k>=0 && k<maxNumComplements);
  assert(j>=0 && j<numHedonicElements);
  assert(k * numHedonicElements + j >=0 && k * numHedonicElements + j < maxNumComplements * numHedonicElements);
  return effectiveComplementaryHedonicElements[k * numHedonicElements + j];
  }
  ...
  void setHedonicQualityElementJ(int j, int a) {
	...
	hedonicQuality[j] = a;
  effectiveQuality[j] = hedonicQuality[j] + artificialQuality[j];
  } //7/15/08

  // 2026-04-15 copilot: keep effective quality synchronized with standard and artificial values.
  //Set the artificial hedonic quality of the firm's good
  void setArtificialQualityElementJ(int j, int a) {
  assert(j>=0 && j<numHedonicElements && a>=0);
  artificialQuality[j] = a;
  effectiveQuality[j] = hedonicQuality[j] + artificialQuality[j];
  }
  ...
  // 2026-04-15 copilot: keep complementary effective qualities synchronized too.
  ...
  effectiveComplementaryHedonicElements[k * numHedonicElements + j] = complementaryHedonicElements[k * numHedonicElements + j] + artificialComplementaryHedonicElements[k * numHedonicElements + j];
  }

  //Set the artificial complementary hedonic quality
  void setArtificialComplementaryHedonicElementKJ(int k, int j, int a) {
  assert(k>=0 && k<maxNumComplements);
  assert(j>=0 && j<numHedonicElements);
  assert(a>=0);
  assert(k * numHedonicElements + j >=0 && k * numHedonicElements + j < maxNumComplements * numHedonicElements);
  artificialComplementaryHedonicElements[k * numHedonicElements + j] = a;
  effectiveComplementaryHedonicElements[k * numHedonicElements + j] = complementaryHedonicElements[k * numHedonicElements + j] + artificialComplementaryHedonicElements[k * numHedonicElements + j];
  }
...
private:
  ...
  int artificialQuality[numHedonicElements];
  int effectiveQuality[numHedonicElements];
  // 2026-04-15 copilot: complement qualities track standard, artificial, and effective forms separately.
  ...
  int complementaryHedonicElements[maxNumComplements * numHedonicElements]; //6/26/09
  int artificialComplementaryHedonicElements[maxNumComplements * numHedonicElements];
  int effectiveComplementaryHedonicElements[maxNumComplements * numHedonicElements];
